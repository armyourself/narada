...
The Lattice node is a long-running daemon that a user (or operator) runs to
participate in the Lattice network. It is responsible for:
...
- **Routing** messages across the Lattice network via other independently
...
- Speaking conventional **IMAP/SMTP** to the local desktop client
  (`client/`) and bridging with `gateway/` for non-Lattice peers (Phase 5+).

## Status

This directory currently contains the Openmail-derived FastAPI server. It
still speaks conventional email (IMAP/SMTP) only — the Lattice protocol layer
