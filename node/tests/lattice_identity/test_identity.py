...
from src.lattice_identity.keypair import (
    generate_keypair,
    keypair_from_seed,
    verify_signature,
)
from src.lattice_identity.mnemonic import (
    generate_mnemonic,
    mnemonic_to_seed,
    validate_mnemonic,
)
...
def test_generate_identity_returns_mnemonic(in_memory_keystore: InMemoryKeystore):
    ...
    assert isinstance(identity, LatticeIdentity)
    assert identity.account_id == "alice@example.com"
    assert identity.public_id.startswith("lattice1")
