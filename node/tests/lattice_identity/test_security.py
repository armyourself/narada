"""Security-focused tests for the Lattice identity layer.
...
* Wrapped ValueError -> LatticeIdentityError on bad X25519/Ed25519 input
"""
...
from src.lattice_identity import (
    InMemoryKeystore,
...
from src.lattice_identity.keystore import _ACCOUNT_ID_PATTERN, _validate_account_id
from src.routers import lattice_identity_tasks as router_module
...
def test_shared_secret_rejects_non_public_id_string() -> None:
    ...
    with pytest.raises(LatticeIdentityError):
        identity.shared_secret_with("not a lattice1... id")
    with pytest.raises(LatticeIdentityError):
...
def test_shared_secret_rejects_low_cardinality_public_id() -> None:
    ...
    identity, _ = generate_identity("alice@example.com", keystore=InMemoryKeystore())
    # A structurally-valid lattice1... id whose X25519 component is all-zero bytes.
    # Build it by encoding 1 + 32 zero bytes + 32 zero bytes.
    import base64
    from src.lattice_identity.encoding import _bech32_encode, _convertbits
    payload = bytes([0x01]) + (b"\x00" * 32) + (b"\x00" * 32)
    data = _convertbits(payload, 8, 5, pad=True)
    bad_id = _bech32_encode("lattice", data, "bech32m")
    with pytest.raises(LatticeIdentityError):
...
def test_router_rejects_unsafe_account_id_on_show() -> None:
    ...
    with TestClient(app) as client:
        ...
        resp = client.get("/lattice/identity/foo%20bar")
        assert resp.status_code == 200
        body = resp.json()
        assert body["success"] is False
...
def test_claim_token_issued_and_consumable() -> None:
    ...
    public_id = "lattice1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq"
...
def test_claim_token_rejects_wrong_public_id() -> None:
    router_module._claim_tokens.clear()
    public_a = "lattice1aaa"
    public_b = "lattice1bbb"
