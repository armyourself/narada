"""Tests for the narada protocol HTTP router."""
...
def app_and_keystore(tmp_path: Path, monkeypatch):
    ...
    manager.add(
        Account(
            ...
            narada_identity_id=bob.public_id,
        )
    )
    ...
    from src.routers import narada_protocol_tasks as router
...
def test_post_narada_inbox_happy_path(app_and_keystore):
    client, alice, bob, _ks, _tmp = app_and_keystore
    body = NaradaBody(subject="hi", body_text="hello", to=("bob@example.com",))
...
def test_post_narada_inbox_rejects_tampered(app_and_keystore):
    client, alice, bob, _ks, _tmp = app_and_keystore
    ...
    resp = client.post("/narada/inbox", json=bad)
    assert resp.status_code == 200
    data = resp.json()
...
def test_post_narada_inbox_rejects_old_timestamp(app_and_keystore):
    client, alice, bob, _ks, _tmp = app_and_keystore
    body = NaradaBody(subject="hi")
...
def test_post_narada_inbox_rejects_unknown_recipient(app_and_keystore):
    ...
    from src.narada_identity.keystore import InMemoryKeystore
    from src.narada_identity.identity import generate_identity
    from src.narada.envelope import make_envelope, NaradaBody
...
def test_post_narada_inbox_idempotent(app_and_keystore):
    client, alice, bob, _ks, _tmp = app_and_keystore
    body = NaradaBody(subject="hi")
    envelope = make_envelope(alice, bob.public_id, body)
    resp1 = client.post("/narada/inbox", json=envelope.as_dict())
    resp2 = client.post("/narada/inbox", json=envelope.as_dict())
    assert resp1.json()["data"]["duplicate"] is False
    assert resp2.json()["data"]["duplicate"] is True


def test_get_narada_inbox_listing(app_and_keystore):
    client, alice, bob, _ks, _tmp = app_and_keystore
    body = NaradaBody(subject="hello", body_text="body text")
    envelope = make_envelope(alice, bob.public_id, body)
    client.post("/narada/inbox", json=envelope.as_dict())
    resp = client.get("/narada/inbox/bob@example.com")
    assert resp.status_code == 200
    body = resp.json()
    ...
    assert msg["source"] == "narada"


def test_outbox_retry_validates_account_id(app_and_keystore):
    client, _alice, _bob, _ks, _tmp = app_and_keystore
    resp = client.post("/narada/outbox/retry", json={"account_id": "../etc/passwd"})
    assert resp.status_code == 200
    assert resp.json()["success"] is False
