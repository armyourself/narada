...
def _link_identity_to_account(account_id: str, public_id: str) -> None:
    ...
    try:
        ...
        if existing is None:
            return
        if existing.lattice_identity_id == public_id:
            return
        manager.edit(
            Account(
...
class GenerateIdentityData(BaseModel):
    ...
    mnemonic_claim_token: str = Field(
        ..., description=(
            ...
            "GET /lattice/identity/{account_id}/mnemonic?token=... . "
...
@router.get("/lattice/identity/{account_id}/mnemonic")
def claim_mnemonic(account_id: str, token: str) -> Response[MnemonicClaimData]:
    ...
    The token is issued by ``/lattice/identity/generate`` or
    ``/lattice/identity/rotate`` and is bound to the public id returned
