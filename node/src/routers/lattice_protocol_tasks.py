"""FastAPI routes for the Lattice protocol (Phase 2 MVP).

The router is the receiver side of the protocol. It accepts a sealed
Lattice envelope over HTTP, validates it, decrypts it, and persists
it to the recipient's account mailbox.

Endpoints:

  * ``POST /lattice/inbox``  - receive an envelope.
  * ``GET  /lattice/inbox/{account_id}``  - list persisted envelopes
    (for debugging / client polling).
  * ``POST /lattice/outbox/retry``  - force-flush a given account's
    pending outbox (debugging / manual retry).

The router is intentionally narrow. The full outbox drain loop
lives in :class:`src.lattice.adapter.LatticeAdapter.drain_outbox`,
...
from ..lattice.adapter import LatticeAdapter
from ..lattice.envelope import LatticeEnvelope, LatticeEnvelopeError
from ..lattice.inbox import LatticeInbox

router = APIRouter(tags=["Lattice Protocol"])
...
@router.post("/lattice/inbox")
def receive_envelope(envelope: Mapping[str, Any]) -> Response:
    """Receive a sealed Lattice envelope from another node."""
    try:
        env = LatticeEnvelope.from_dict(envelope)
...
@router.get("/lattice/inbox/{account_id}")
def list_inbox(account_id: str) -> Response:
    ...
    messages = adapter.fetch_messages("Lattice", limit=200, offset=0)
    return Response(
        success=True,
...
@router.post("/lattice/outbox/retry")
...
def _account_id_for_public_id(public_id: str) -> Optional[str]:
    ...
    adapter loads the Lattice identity directly from the keystore
    using the account_id. The :class:`AccountManager` carries
    ``lattice_identity_id`` on each account (set by the
    lattice_identity router on generation), so we walk the
    ``AccountManager`` here.
    """
    ...
    for account in accounts:
        if account.lattice_identity_id == public_id:
