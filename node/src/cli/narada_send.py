"""CLI: send a Narada message from the command line.

Usage:

    python -m src.cli.narada_send <account_id> <recipient_public_id> \\
        --subject "hi" --body "hello"

The Narada identity for ``account_id`` is loaded from the keystore
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="Narada-send",
        description="Send a Narada message from the command line.",
    )
    p.add_argument("account_id", help="local account whose Narada identity to use")
    p.add_argument("recipient_public_id", help="recipient's narada1... public id")
...
def main(argv: list[str] | None = None) -> int:
    ...
    adapter = NaradaAdapter(args.account_id, keystore=default_keystore())
