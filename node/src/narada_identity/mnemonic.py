"""BIP-39 mnemonic support for Narada identity recovery.
...
from .errors import NaradaMnemonicError
...
def generate_mnemonic(strength_bits: int = _ENTROPY_BITS) -> str:
    ...
    if strength_bits != _ENTROPY_BITS:
        raise NaradaMnemonicError(
            f"Narada identities only support {_ENTROPY_BITS}-bit mnemonics "
...
def mnemonic_to_seed(phrase: str) -> bytes:
    """Convert a BIP-39 mnemonic to the 32-byte Narada Ed25519 seed.
    ...
    if not validate_mnemonic(phrase):
        raise NaradaMnemonicError("Invalid BIP-39 mnemonic")
