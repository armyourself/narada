"""Lattice directory (Phase 2 MVP).

A :class:`LatticeDirectory` maps a recipient's ``lattice1...`` public
id to a base URL where the recipient's node accepts envelopes (the
``/lattice/inbox`` endpoint). Today the only implementation is
...
class LocalContactList(LatticeDirectory):
    ...
    File format::

        {
          "version": 1,
          "contacts": [
            {"public_id": "lattice1...", "base_url": "http://127.0.0.1:8123",
...
class InMemoryDirectory(LatticeDirectory):
