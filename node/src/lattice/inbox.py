"""Receiver-side helpers for incoming Lattice envelopes (Phase 2 MVP).

A :class:`LatticeInbox` is the thing the HTTP router hands a raw
envelope to. It:

  1. Verifies the sender is known and the signature is valid
     (delegated to :func:`src.lattice.envelope.open_envelope`).
  2. Decrypts the body.
  3. Persists the message into the recipient's account store, with
     ``source='lattice'`` so the rest of the Lattice node (and the
     desktop client) can render it as a Lattice-delivered message.
...
class LatticeInbox:
    """Receiver-side helper for Lattice messages.

    ``account_id`` is the local account that owns this inbox. The
    recipient's Lattice identity is loaded from ``keystore`` on the
    ...
    def load_identity(self) -> LatticeIdentity:
        """Load the recipient's Lattice identity for this account."""
        ...
        try:
            ...
            raise LatticeEnvelopeError(
                f"account {self._account_id!r} has no Lattice identity"
            ) from exc
    ...
    def _persist(self, envelope: LatticeEnvelope, body: LatticeBody) -> None:
        ...
        record = {
            ...
            "flags": ["\\Seen"],
            "attachments": [],
            "message_id": f"<{envelope.message_id}@lattice>",
